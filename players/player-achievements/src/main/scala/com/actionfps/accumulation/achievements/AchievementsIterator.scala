...
object AchievementsIterator {
  def empty = AchievementsIterator(userToState = Map.empty, events = List.empty)
}

/**
  * Iterator combining all user's data.
  */
case class AchievementsIterator(userToState: Map[String, PlayerState], events: List[Map[String, String]]) {
  ...
  def newAchievements(updatedPlayers: Map[String, PlayerState],
                      previous: AchievementsIterator): List[(String, List[(String, CompletedAchievement)])] = {
    (for {
  ...
  def includeGame(users: List[User])(jsonGame: JsonGame): (AchievementsIterator, Map[String, PlayerState]) = {
    ...
    for {
      ...
      (newPs, newEvents) <- userToState.getOrElse(user.id, PlayerState.empty).includeGame(jsonGame, team, player)(p =>
        users.exists(_.validAt(p.name, jsonGame.endTime)))
    } {
      oEvents ++= newEvents.map { case (date, text) =>
        Map("user" -> user.id, "date" -> date, "text" -> s"${user.name} $text")
      }
      updates += (user.id -> newPs)
    }

    (copy(userToState = userToState ++ updates, events = oEvents.toList ++ events), updates.toMap)
  }
}
