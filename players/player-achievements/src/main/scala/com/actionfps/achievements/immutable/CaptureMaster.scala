...
object CaptureMaster {
  ...
  case class Achieving(achieving: List[CaptureMapCompletion.Achieving],
    ...
    def includeGame(jsonGame: JsonGame,
                    ...
                    jsonGamePlayer: JsonGamePlayer): Option[
      (Either[Achieving, Achieved], Option[CaptureMapCompletion.Achieved])] = {
      val withIncluded = achieving.map(a =>
        a.include(jsonGame, jsonGameTeam, jsonGamePlayer).getOrElse(Left(a)))
      val nextMe = copy(
      ...
      if (nextMe == this) Option.empty
        ...
        Option {
          val newlyCompleted =
            (nextMe.achieved.toSet -- achieved.toSet).headOption
          val myNextIteration =
    ...
    override def progress: Int =
      if (achieved.isEmpty) 0
      else 100 * achieved.length / (achieved.length + achieving.length)
