...
class FullFlowTest
  ...
  test("Games are written in properly") {
    ...
    val clans = ClanRecord.parseRecords(com.actionfps.reference.getSample("clans.csv")).map(Clan.fromClanRecord)
    val nicknames = NicknameRecord.parseRecords(com.actionfps.reference.getSample("nicknames.csv"))
    val users = Registration.parseRecords(com.actionfps.reference.getSample("registrations.csv")).flatMap(User.fromRegistration(_, nicknames))
