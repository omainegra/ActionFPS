...
case class WinFlow(challongeClient: ChallongeClient)(implicit executionContext: ExecutionContext) {
  private def clanwar: Flow[CompleteClanwar, Int, NotUsed] = {
    Flow[CompleteClanwar]
      ...
      .mapConcat(_.toList)
      .mapAsync(3)(Function.tupled(challongeClient.attemptSubmit(WinFlow.TestClanwarTournament, _, _, _, _)))
      .mapConcat(_.toList)
  }
  ...
  private def game: Flow[JsonGame, Int, NotUsed] = {
    Flow[JsonGame]
      ...
      .mapConcat(_.toList)
      .mapAsync(3)(Function.tupled(challongeClient.attemptSubmit(WinFlow.TestGameTournament, _, _, _, _)))
      .mapConcat(_.toList)
  }
  ...
  def clanwarAny: Flow[CompleteClanwar, Int, NotUsed] = {
    Flow[CompleteClanwar]
      .mapConcat(ccw => WinFlow.detectWinnerLoserClanwar(ccw).toList.map(s => ccw -> s))
