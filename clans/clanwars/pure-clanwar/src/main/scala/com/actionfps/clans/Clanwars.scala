...
case class Clanwars(incomplete: Set[IncompleteClanwar],
  ...
  privatedef includeGame(jsonGame: Game): Option[(Clanwars, Option[CompleteClanwar])] = {
    incomplete.flatMap(ic => ic.potentialNextGame(jsonGame).map(n => ic -> n)).headOption.map {
      case (ic, Left(nc)) =>
        copy(incomplete = incomplete - ic + nc)-> None
      case (ic, Right(cc)) =>
        copy(incomplete = incomplete - ic, complete = complete + cc) -> Some( cc)
    } orElse {Clanwar.begin(jsonGame).map(cw => copy(incomplete = incomplete + cw)-> None)
    }
  }

  /**
    * New state, + a complete clanwar if possible
    */def includeFlowing(jsonGame: Game): (Clanwars , Option[CompleteClanwar]) = includeGame(jsonGame).getOrElse(this-> None)
