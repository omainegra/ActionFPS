...
object GameStartHeader {
  private val timeRemaining = CharIn('0' to '9')
    .rep(1)
    .!
    .map(_.toInt) ~ " " ~ ("minutes" | "minute")
...
case class GameStartHeader(mode: GameMode.GameMode,
                           map: String,
                           players: Int,
                           minutes: Int)
...
/**
  ...
  */
case class GameFinishedHeader(mode: GameMode.GameMode,
                              map: String,
                              state: String)
...
case class GameInProgressHeader(mode: GameMode.GameMode,
                                remaining: Int,
                                map: String,
                                state: String)
...
object GameInProgressHeader {
  ...
  private val timeRemaining = CharIn('0' to '9')
    .rep(1)
    .!
    .map(_.toInt) ~ " " ~ ("minutes" | "minute") ~ " remaining"
  ...
  private val cap = P("Game status:") ~ spaces ~ SharedParsers.acceptedModeParser ~ " on " ~ SharedParsers.mapNameParser.! ~
    ", " ~ timeRemaining ~ "," ~ spaces ~ CharsWhile(_ != ',')
    .rep(1)
    .! ~ ", " ~ clients

  private val cap2 = cap.map {
    case (mode, map, remain, state, clientCount) =>
      GameInProgressHeader(mode, remain, map, state)
  }
...
object TeamModes {
  ...
  object FragStyle {
    ...
    case class IndividualScore(cn: Int,
                               name: String,
                               team: String,
                               score: Int,
                               frag: Int,
                               ...
                               user: Option[String],
                               group: Option[String])
        extends CreatesGenericIndividualScore {
      override def project: GenericIndividualScore =
        GenericIndividualScore(name,
                               team,
                               None,
                               Option(score),
                               frag,
    ...
    object IndividualScore {
      ...
      private val cpp = cap.map {
        case (a, b, c, d, e, f, g, h, i, j) =>
          IndividualScore(a, b, c, d, e, f, g, h, i, j.host, j.userO, j.group)
      }
    ...
    case class IndividualScoreDisconnected(name: String,
                                           team: String,
                                           frag: Int,
                                           death: Int)
        extends CreatesGenericIndividualScore {
      override def project: GenericIndividualScore = GenericIndividualScore(
        ...
        None,
        None,
        None // todo figure out how to get user & group for disconnected.
      )
    }

    object IndividualScoreDisconnected {
      private val isdp = " ".rep ~ strSp ~ strSp ~ dSp ~ dSp ~ " ".rep ~ "-" ~ " ".rep ~ "-" ~ " ".rep ~ "disconnected"
      private val isd = isdp.map {
        case (a, b, c, d) => IndividualScoreDisconnected(a, b, c, d)
      }
    ...
    case class TeamScore(teamName: String, players: Int, frags: Int)
        extends CreatesGenericTeamScore {
      override def project: GenericTeamScore =
        GenericTeamScore(teamName, players, None, frags)
    }
  ...
  case class GenericTeamScore(name: String,
                              players: Int,
                              flags: Option[Int],
                              frags: Int)
  ...
  case class GenericIndividualScore(name: String,
                                    team: String,
                                    flag: Option[Int],
                                    score: Option[Int],
                                    frag: Int,
  ...
  object FlagStyle {
    ...
    object IndividualScoreDisconnected {
      private val cpp = " ".rep ~ strSp ~ strSp ~ dSp ~ dSp ~ dsp ~ " ".rep(1) ~ "-" ~ " "
        .rep(1) ~ "-" ~ " ".rep(1) ~ "disconnected"
      private val cmp = cpp.map {
        case (a, b, c, d, e) => IndividualScoreDisconnected(a, b, c, d, e)
      }
    ...
    case class TeamScore(name: String, players: Int, frags: Int, flags: Int)
        extends CreatesGenericTeamScore {
      override def project: GenericTeamScore =
        GenericTeamScore(name, players, Option(flags), frags)
    }
    ...
    object TeamScore {
      ...
      private val trp = tpp.map {
        case (n, p, f, fl) => TeamScore(n, p, f, fl)
      }
