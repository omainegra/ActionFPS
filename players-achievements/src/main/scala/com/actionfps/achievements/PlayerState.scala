...
case class PlayerState(
    ...
    combined
      ...
      .map {
        case (newCombined, newEvents, newAchievements) =>
          val newEventsT = newEvents.map(eventText =>
            GameEvent(gameId = jsonGame.id, eventText = eventText))
          val newMe = copy(
      ...
      case None if nps != playerStatistics =>
        Option(copy(playerStatistics = nps) -> Vector.empty)
      case None => None
    }
  }
  ...
  def buildAchievements: AchievementsRepresentation = {
    AchievementsRepresentation(
      completedAchievements = achieved
        .sortBy { case (date, achievement) => date }
        .reverse
        .map {
          case (date, achievement) =>
        ...
        .toList,
      partialAchievements = combined.combined
        .collect {
...
case class AchievementsRepresentation(
    completedAchievements: List[CompletedAchievement],
    partialAchievements: List[PartialAchievement],
    switchNotAchieveds: List[SwitchNotAchieved])

case class CompletedAchievement(title: String,
                                description: String,
                                at: String,
                                captureMaster: Option[CaptureMaster])

case class PartialAchievement(title: String,
                              description: String,
                              percent: Int,
                              captureMaster: Option[CaptureMaster])
