...
import ac.woop.client.MasterClient.Repository.{UserData, UserServer, UserId, Server}
...
class KeyExchanger(service: ActorRef, remote: PeerId, server: Server, serverUsers: Map[UserId, UserServer]) extends Act with ActorLogging {
  whenStarting {
    ...
    service ! SendMessageAddition(remote, 0)(10, serverUsers.size)
    for {
    ...
    } service ! SendMessageAddition(remote, 0)(bitums.foldLeft(ByteString(11.toByte))(_ ++ _), ByteString.empty)
    log.info("All {} users sent", serverUsers.size)
    service ! SendMessageAddition(remote, 0)(12, ByteString.empty)
  }
  become {
